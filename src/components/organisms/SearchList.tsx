...
export default function SearchList({ count: initialCount, error: countError, loading: countLoading, endpoint, title, internal, cta}: Props) {
  ...
  const getResults = async () => {
    ...
    setLoading(true)
    setError(null)
    const { data, error }: FavoritesResponse = await request(`${endpoint}?page=${page}&q=${debouncedValue}&perPage=${itemsPerPage}`)
  ...
  return (
    <>
      <h1 className="font-display text-center text-3xl font-bold text-brand-alt mb-3">{title}</h1>
      <div className="w-full py-4">
        <form className="flex flex-col-reverse justify-center md:flex-row md:items-center gap-4" onSubmit={e => {
          e.preventDefault()
          getResults()
        }}>
          <input
            ...
            onChange={onChange}
          />
          <div className="whitespace-nowrap font-semibold text-center text-sm md:text-base self-stretch">
            {cta || <Button className="h-full" type="submit" icon={<MagnifyingGlassIcon className="w-5" />} />}
          </div>
        </form>
      </div>
      <div className={classNames('-mx-2 grid grid-cols-2 gap-3 gap-y-6 md:gap-0 md:gap-y-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 justify-start', maxPageCount === 1 && 'mb-8')}>
        {!loading && !!results?.length && (
          results.map((recipe: SupaRecipe) => (
            <RecipeCard internal={internal} key={recipe.id} recipe={recipe} className="md:px-2 w-full sm:w-full md:w-full max-w-full" />
          ))
        )}
        {loading && (
        ...
        {initted && !!results?.length && (
          <div className="border-t border-t-gray-200 flex col-span-2 sm:col-span-3 md:col-span-4 lg:col-span-5 gap-4 justify-center my-4 md:mx-3 pt-4">
            <div className="grow text-xs text-slate-500">
              {!!count && (<>{count} recipe{count !== 1 && 's'}</>)}
            </div>
            {maxPageCount > 1 && (
      ...
      {initted && !count && !!debouncedValue && (
        <RecipeError className="my-8" errorText="We couldn't find any recipes that matched your search." />
      )}
    </>
  )
}